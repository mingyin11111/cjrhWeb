            ...
            <el-row>
                <el-col :span="24">
                    <div class="grid-content bg-purple-dark">
                        <div style="display: inline; cursor: pointer">专项竞赛&gt;</div>
                        <div style="
                  display: inline;
                  float: right;
                  margin-top: -4px;
                  ...
                  cursor: pointer;
                ">
                            <el-button type="primary" size="mini" @click="To_Add">添加</el-button>
                        </div>
                    </div>
                    <template>
                        <el-table :data="pageinfo.PageData" style="width: 100%" row-key="id" :row-style="rowStyle"
                            ...
                            <el-table-column prop="封面" label="封面" width="120">
                                <template slot-scope="scope">
                                    <el-image v-if="scope.row.Pic"
                                        ...
                                        :src="scope.row.Pic.split('*')[0]" fit="scale-down"></el-image>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column prop="Abstract" label="摘要">
                                <template slot-scope="scope">
                                    {{ formatLen(scope.row.Abstract, 130) }}
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column fixed="right" label="操作" width="150" align="center">
                                <template slot-scope="scope">
                                    <el-button @click="EditRow(scope.row)" type="text" size="medium">
...
export default {
    ...
    methods: {
        EditRow(row) {
            this.$router.push({ path: '/Manager/Competition/Add', query: { id: row.id } })
        },

        To_Add() {
            this.$router.push({ path: '/Manager/Competition/Add' })
        },
        ...
        deleteRow(row) {
            this.$confirm("确定要删除该项竞赛信息吗？", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning",
                center: true,
            }).then(() => {
                this.deleteCompetition(row);
            })
                .catch(() => {
                    this.$message({
                        type: "info",
