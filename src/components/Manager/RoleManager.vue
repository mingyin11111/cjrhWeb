      ...
      <el-row>
        <el-col :span="24">
          <div class="grid-content bg-purple-dark">
            <div style="display: inline; cursor: pointer">角色定义&gt;</div>
            <div
              style="
                display: inline;
                float: right;
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    @click="ToMemberManager(scope.row)"
                    type="text"
                    size="medium"
                  >
...
export default {
  ...
  methods: {
      ToMemberManager(row)
      {
        this.$router.push("/Manager/RoleMemberManager/" + row.id+"/"+row.label);
      },
    Config(row) {
    ...
    Edit_Role(row) {
      this.$axios
        .get("/api/systemRole/"+row.id, {
          params: {
          },
        })
        .then((response) => {
          if (response.data.data.err > 0) {
            this.$message.error("Error:"+response.data.data.err);
          } else {
    ...
    doUpdate() {
      ...
      this.$axios({
        method: "patch",
        url: "/api/systemRole/"+this.currentRow.id,
        data: qs.stringify({
          Name: this.module.Name,
          IndexNumber: this.module.IndexNumber,
          Memo: this.module.Memo,
